...
# save the model
model.save("./model1_store/sac_pendulum")

# the saved model does not contain the replay buffer
loaded_model = SAC.load("./model1_store/sac_pendulum")
print(f"The loaded_model has {loaded_model.replay_buffer.size()} transitions in its buffer")

# now save the replay buffer too
model.save_replay_buffer("./model1_store/sac_replay_buffer")

# load it into the loaded_model
loaded_model.load_replay_buffer("./model1_store/sac_replay_buffer")

# now the loaded replay is not empty anymore
print(f"The loaded_model has {loaded_model.replay_buffer.size()} transitions in its buffer")

loaded_model.learn(1000)
