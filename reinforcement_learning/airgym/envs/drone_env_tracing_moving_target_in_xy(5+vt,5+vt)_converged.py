...
class AirSimDroneEnv(AirSimEnv):
    ...
    #这个是无人机飞向移动目标的计算奖励，仅在x-y平面上 额外设置经验（0.3，0.3，0） 目前已经收敛
    @property
    def _compute_reward(self):
        ...
        diff = now_time - start_episode_time
        diff_seconds = diff.total_seconds()
        print(f"两时间相差 {diff_seconds} 秒")
        ...
        #解析移动目标的位置
        target_x = 5 + diff_seconds*0.1
        target_y = 5 + diff_seconds*0.1
        print(f"the diff position between is ( {target_x - drone_x},{target_y - drone_y} )")
        if (drone_x - target_x >= 2) or (drone_y - target_y >= 2 ) or (drone_x < -10) or (drone_y < -10) or drone_z < -12:
            reward_dist = 30 # x,y超过目标1米,
        else:
            reward_dist = math.sqrt(pow(target_x - drone_x, 2) + pow(target_y - drone_y, 2))
