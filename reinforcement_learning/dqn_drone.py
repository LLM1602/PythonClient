...
dir = "E:/PyCharmProject/llm_AirSim_github_modified/PythonClient/reinforcement_learning/model_store/dqn_drone/best_model"
...
checkpoint_callback = CheckpointCallback(save_freq=500, save_path='./model_store/dqn_drone_step/',
                                         name_prefix='dqn_tracing_model_in_xy_no_special')
checkpoint_callback1 = CheckpointCallback1(save_freq=500, save_path='./model_store/dqn_drone_step/',
                                         name_prefix='dqn_tracing_model_in_xy_buffer_no_special')
...
file_name = 'record/dqn_drone/recording_closing_position_static_not_special(0.3,0.3).txt'
...
if os.path.exists(dir+".zip"):
    print("已有模型，正在加载模型")
    model = DQN.load("./model_store/dqn_drone/best_model",env,device="cuda")
